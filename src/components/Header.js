  ...
  return (
    <header className={`${isActive
      ...
      <div className='flex justify-between items-center h-full pl-[50px] pr-[60px]'>
        <a href='/'>
          <img className='w-[188px] h-[190px]' src={logo} alt='' />
        </a>
        <div className='hidden xl:flex'>
          <Nav />
        </div>
        <div onClick={() => setNavMobile(!navMobile)} className='xl:hidden absolute right-[5%] bg-dark p-2 rounded-md cursor-pointer'>
          <TiThMenuOutline className='text-3xl text-white' />
        </div>
        <div className={`${navMobile ? 'max-h-full' : 'max-h-0 overflow-hidden'} ${isActive
